...
class FilerMover():
    ...
    def __init__(self):
        self.log = LoggerSetup().get_logger('file_mover')
        ...
        for file in file_list:
            try:
                if file.split(".")[1] in file_extensions:
                    ...
                    if result is None:
                        self.log.warning('{} has nothing to process. File has been moved to {} directory.'.format(file, ERROR_PATH))
                        os.rename(FROM_PATH + file, ERROR_PATH + file + 'EmptyFileError')
                        continue
                    elif isinstance(result, tuple) and sum(1 for rec in result) != val_num:
                        self.log.warning("{} has bad format, and can't be process. File has been moved to {} directory.".format(file, ERROR_PATH))
                    ...
                    self.log.info('Moving {} to {} directory.'.format(file, TO_PATH))
                    os.rename(FROM_PATH + file, TO_PATH + file)
                    sleep(0.5)
                    if path.exists(TO_PATH + file):
                        self.log.info("{} moved succesfully.".format(file))
                        sleep(0.5)
                else:
                    self.log.warning('{} Files with this extension are not supported. File has been moved to {} directory.'.format(file, ERROR_PATH))
                    os.rename(FROM_PATH + file, ERROR_PATH + file + 'ExtensionError')
                    sleep(0.5)
            except IndexError:
                self.log.warning('{} Files without extension are not supported. File has been moved to {} directory.'.format(file, ERROR_PATH))
                os.rename(FROM_PATH + file, ERROR_PATH + file + 'ExtensionError')
                sleep(0.5)
    ...
    def mysql_insert(self, result):
        self.log.debug('Calling mysql_insert() method.')
        try:
            ...
            if conn.is_connected():
                try:
                    ...
                    if isinstance(result, list):
                        ...
                        self.log.warning('Bad format for request.')
                    conn.commit()
                    self.log.info('{} record inserted.'.format(cur.rowcount))
                except Error as err:
                    self.log.error('An error occured while executing a sql query: {}'.format(err), exc_info=True)
                finally:
                    if conn.is_connected():
                        cur.close()
                        conn.close()
        except Error as err:
            self.log.error('An error occured while connecting to database: {}'.format(err), exc_info=True)
            result=[]
            return result
    ...
    def read_file(self, file, delimiter):
        self.log.debug('Calling read_file() method on "{}" with delimiter "{}".'.format(file.split('/')[-1], delimiter))
        result=[]
        try:
            ...
            self.log.error('An error occured while reading a file: {}'.format(fnerr), exc_info=True)
